...
class WriteToObjectStorage:
    ...
    def downloadDeltaFiles(self):
        for file_name in self.responseText:
            #print(self.__file_link.format(filename=filename))
    ...
    def __getDeltaFile(self, file_name, file_url):
        ...
        s3_response = self.__s3_client.put_object(Bucket=self.__bucket_name, Key=s3_file_name, Body=response.content)
        print(f"Uploaded {file_name} from {file_url} with {s3_response}")

#WriteToObjectStorage().list_s3_objects()